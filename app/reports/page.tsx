  ...
  return (
    <>
      ...
      <main className="flex-1 overflow-y-auto custom-scrollbar p-8 pb-32 relative bg-[#050505]">
        ...
        <div className="mb-10 flex items-end justify-between relative z-[100] transition-all">
          <div>
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              <button 
                ...
                className="flex items-center gap-2 bg-black/40 border border-white/10 hover:border-white/20 text-white px-4 py-2.5 rounded-xl transition-all hover:bg-white/5"
              >
                <Calendar className="w-4 h-4 text-white/60" />
                <span className="text-sm font-medium">{dateRange}</span>
                <ChevronDown className="w-3.5 h-3.5 text-white/40 ml-1" />
              </button>
              <div className={`absolute top-full right-0 mt-2 w-48 glass-card bg-[#0A0A0A] border-white/10 transition-all z-50 p-2 shadow-2xl ${isFilterOpen ? "opacity-100 visible" : "opacity-0 invisible"}`}>
                {["Last 24 Hours", "Last 7 Days", "Last 30 Days"].map(range => (
                  <button 
                    key={range} 
                    onClick={() => { changeRange(range); setIsFilterOpen(false); }} 
                    className={`w-full text-left px-4 py-2 text-sm rounded-lg transition-colors ${dateRange === range ? "text-amber-500 bg-amber-500/5" : "text-white/60 hover:text-white hover:bg-white/5"}`}
                  >
                    {range}
                  </button>
                ))}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 relative z-10 min-h-[450px]">
              <div className="lg:col-span-2 glass-card p-8 flex flex-col h-full group border-white/5 border-t-[3px] border-t-amber-500 shadow-2xl">
                <div className="flex items-center justify-between mb-8">
                  <h2 className="text-xl font-outfit font-medium text-white tracking-wide uppercase">Revenue Trend</h2>
                  <div className="text-[10px] font-bold text-white/20 bg-black/40 px-3 py-1.5 rounded-lg border border-white/10 uppercase tracking-widest">{dateRange}</div>
                </div>
                    ...
                    return (
                      <div key={i} className="flex flex-col items-center gap-4 flex-1 group/bar cursor-crosshair">
                        <div className="w-full relative h-[250px] bg-black/20 rounded-t-xl group-hover/bar:bg-black/30 transition-colors border-x border-t border-white/5 overflow-hidden flex items-end">
                          <motion.div initial={{ height: 0 }} animate={{ height: `${Math.max(2, (d.total / max) * 100)}%` }} transition={{ delay: i * 0.05, duration: 1, type: "spring" }} className="w-full rounded-t-lg bg-gradient-to-t from-black/10 to-amber-500/60 shadow-glow shadow-amber-500/10 group-hover/bar:brightness-125" />
                        </div>
                        <div className="text-[9px] font-bold text-white/20 group-hover/bar:text-white transition-colors uppercase tracking-[0.1em] whitespace-nowrap overflow-hidden">{d.day}</div>
                      </div>
                    );
                  })}
