...
export async function getRevenueAnalytics(range: string = "This Week") {
  const supabase = await createClient(); const { restaurantId } = await verifySession(supabase);
...
export async function addInventoryItem(item: { name: string; qty: number; unit: string; min_stock: number; supplier?: string; category?: string; }) {
  ...
  const supabase = await createClient(); const { restaurantId } = await verifySession(supabase);
  const { data, error } = await supabase
    .from('inventory').insert({ ...item, restaurant_id: restaurantId, display_id, status }).select();
...
export async function deleteInventoryItem(id: string) {
  const supabase = await createClient(); const { restaurantId } = await verifySession(supabase);
  const { error } = await supabase
    .from('inventory').delete().eq('id', id).eq('restaurant_id', restaurantId);
  if (error) throw error;
}
...
export async function createPurchaseOrder(po: { supplier: string; requirements: string; }) {
  ...
  const { error } = await supabase
    ...
    .insert({ ...po, restaurant_id: restaurantId });
  if (error) throw error;
}
...
export async function deleteStaffMember(id: string) {
  const supabase = await createClient(); const { restaurantId } = await verifySession(supabase);
  const { error } = await supabase
    .from('staff').delete().eq('id', id).eq('restaurant_id', restaurantId);
  if (error) throw error;
}
...
export async function updateOrderStatus(id: string, status: string) {
  ...
  if (status === 'served') {
    ...
    if (ord?.table_label) {
      await supabase.from('tables').update({ status: 'free' }).eq('display_id', ord.table_label).eq('restaurant_id', restaurantId);
    }
  }
...
export async function deleteOrder(id: string) {
  ...
  const { error } = await supabase
    .from('orders').delete().eq('id', id).eq('restaurant_id', restaurantId);
  if (error) throw error;
}
...
export async function createReservation(res: { customer_name: string; party_size: number; reservation_date: string; reservation_time: string; table_id?: string; phone?: string; email?: string; notes?: string; status?: string; }) {
  const supabase = await createClient(); const { restaurantId } = await verifySession(supabase);
  const { data, error } = await supabase
    .from('reservations').insert({ ...res, restaurant_id: restaurantId }).select();
...
export async function deleteReservation(id: string) {
  const supabase = await createClient(); const { restaurantId } = await verifySession(supabase);
  const { error } = await supabase
    .from('reservations').delete().eq('id', id).eq('restaurant_id', restaurantId);
  if (error) throw error;
}
